...
function tmR(tmW) {
  ...
  var tmZ = document.images["j_img" + this.tm4.tmC + "_" + this.tmC],
    tma = document.images["i_img" + this.tm4.tmC + "_" + this.tmC];
...
function tmS(tmb) {
  ...
  if (tma) tma.src = this.tmP();
  tmY("i_txt" + this.tm4.tmC + "_" + this.tmC).style.fontWeight = tmb
    ? "normal"
    : "bold";
  this.tmI();
...
function tmU(tmd) {
  window.setTimeout(
    'window.status="' +
      (tmd ? "" : this.tm3[0] + (this.tm3[1] ? " (" + this.tm3[1] + ")" : "")) +
      '"',
    10
  );
}
...
function tmT() {
  ...
  for (var i = this.tm7; i > 1; i--) {
    tme[i] =
      '<img src="' +
      this.tm4.tm2[tmf.tmV() ? "icon_e" : "icon_l"] +
  ...
  return (
    ...
    '<a href="' +
    this.tm3[1] +
    '" target="principal" onclick="return trees[' +
    this.tm4.tmC +
    "].tmG(" +
    this.tmC +
    ')" ondblclick="trees[' +
...
function tmQ(tmg) {
  return this.tm4.tm2[
    "icon_" +
      ((this.tm7 ? 0 : 32) +
        (this.tmK.length ? 16 : 0) +
        (this.tmK.length && this.tmF ? 8 : 0) +
        (!tmg && this.tm4.tm6 == this ? 4 : 0) +
        (tmg ? 2 : 0) +
        (tmg && this.tmV() ? 1 : 0))
  ];
} //function
